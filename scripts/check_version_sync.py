...
def find_hardcoded(dist: str, quelle: Path | None = None) -> list[tuple[str, int, str]]:
    """Manuell gepflegte Versionen in `src/`.

    `quelle` ist die Datei, aus der hatch die dynamische Version liest. Dort
    ist das Literal die Wahrheit und kein Befund — überall sonst bleibt es
    einer.
    ...
    for path in sorted(SRC.rglob("*.py")):
        if quelle is not None and path == quelle:
            continue
...
def dynamic_source() -> tuple[Path, str] | None:
    """(Datei, Version) der hatch-Versionsquelle — oder None.

    `[tool.hatch.version] path = "..."` zeigt auf die Datei, in der
    `__version__` steht. Das ist bei dynamischer Version die einzige Stelle,
    an der die Zahl von Hand gepflegt wird — und damit genau die, gegen die
    `server.json` abgeglichen gehoert.
    """
    text = PYPROJECT.read_text(encoding="utf-8")
    block = re.search(
        r"^\[tool\.hatch\.version\]\s*$(.*?)(?=^\[|\Z)",
        text,
        re.MULTILINE | re.DOTALL,
    )
    if block is None:
        return None
    hit = re.search(r'^\s*path\s*=\s*"([^"]+)"', block.group(1), re.MULTILINE)
    if hit is None:
        return None
    datei = ROOT / hit.group(1)
...
def main() -> None:
    ...
    if version is None:
        # `dynamic = ["version"]`. Frueher stieg der Check hier aus — und
        # liess damit ausgerechnet die Repos ungeprueft, in denen die Zahl
        # *nicht* in pyproject.toml steht und beim Anheben am ehesten
        # zurueckbleibt. Aufloesen statt ueberspringen.
        aufgeloest = dynamic_source()
        if aufgeloest is None:
            print(
                "Versions-Sync FEHLGESCHLAGEN: pyproject.toml deklariert die "
                "Version als dynamisch, aber die Quelle ist nicht auffindbar.",
                file=sys.stderr,
            )
            print(
                '\nErwartet wird `[tool.hatch.version] path = "..."` mit einem '
                "`__version__` in der genannten Datei. Ein stilles Bestehen "
                "waere hier das Schlechteste: der Check liefe, ohne zu pruefen.",
                file=sys.stderr,
            )
            sys.exit(1)
        quelle, version = aufgeloest
    ...
    hardcoded = find_hardcoded(dist, quelle)
    if hardcoded:
